...
var sel_time_data = function (per_page) {
    ...
    status = $('#status').val();    
    var url = '<?php echo site_url("ctl=event&act=event_check");?>'+"&is_event=1&user_id="+user_id+"&event_month="+event_month+"&status="+status;
    var getobj = {};
    if(per_page>0){
...
$(function() {
        ...
        $(".do_search").click(function() {
            ...
            var url = "<?php echo site_url(array('ctl'=>'event', 'act'=>'event_check'))?>"+"&is_event=1&user_id="+user_id+"&event_month="+event_month+"&status="+status;
            window.location.href = url;
            /**
            $.ajax({
                type: "POST",
